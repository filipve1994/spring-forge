...
Spring Forge is library template that lets you extend Spring Boot so to speak for your company or personal needs. In my
career, I have been tasked with improving Spring Boot application development at the company on more than one occasion.

From my experience, handling dependency and plugin management in one place across teams works best. This is something I
find many
...
- You want to create custom Spring Boot autoconfiguration and starters to handle cross-cutting concerns for your Spring
  Boot applications
- You want to scaffold new Spring Boot applications that are production ready in a quick and consistent manner, so you
  can get down to coding the business logic
...
|                       Module                        |                                             Description                                             | Spring Forge Maven Group Name |
|-----------------------------------------------------|-----------------------------------------------------------------------------------------------------|-------------------------------|
| [Spring Forge Starter POM](pom.xml)                 | The root POM that is intended to be used as the parent POM in applications or libraries.            | dev.filipvde.springforge      |
| [autoconfigure](autoconfigure)                      | This module contains custom Spring Boot autoconfiguration and its supporting classes.               | dev.filipvde.springforge      |
...
Add a server reference in your Maven `settings.xml` to get GitHub repository. GitHub hosted packages require a GitHub
username and GitHub token to access them even for read only access.
...
ℹ️ See the [Spring Forge Template](https://github.com/filipve1994/spring-forge-service-template) for a complete example.
...
This autoconfiguration is enabled by default if the `filipvde-spring-boot-starter-app` is on the classpath. It applies
the Spring Controller Advice found
in [CommonControllerAdvice.java](autoconfigure/src/main/java/dev/filipvde/springforge/autoconfigure/controlleradvice/CommonControllerAdvice.java).
...
This autoconfiguration is enabled by default if the `filipvde-spring-boot-starter-app` is on the classpath. The
`filipvde-spring-boot-starter-app` includes `filipvde-spring-boot-starter-logging` which bundles . It's only
...
- [workflows/continuous-integration.yml](.github/workflows/continuous-integration.yml) - Workflow for continuous
  integration
...
- [dependabot.yml](.github/dependabot.yml) -
  A https://docs.github.com/en/code-security/getting-started/dependabot-quickstart-guide configuration
